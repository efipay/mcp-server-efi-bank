...
import { z } from 'zod';
import { PixCreateChargeBody, PixCreateChargeParams } from '../../../types/apis/index.js';
import { McpResponse, McpTextContent } from '../../../types/mcp.js';
import { EfiSDK } from '../../index.js';

export const pixCreateChargeTool = {
  name: TOOLS_CONFIG.pixCreateCharge.name,
  description: TOOLS_CONFIG.pixCreateCharge.description,
  parameters: {
    params: z
      .object({
        txid: z
  ...
  handler: async ({
    ...
    params: PixCreateChargeParams;
    body: PixCreateChargeBody;
  }): Promise<McpResponse> => {
    try {
      const result = await EfiSDK.pixCreateCharge(params, body);
