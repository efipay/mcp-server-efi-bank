...
import { z } from 'zod';
import { PixDeleteWebhookBody, PixDeleteWebhookParams } from '../../../types/apis/index.js';
import { McpResponse, McpTextContent } from '../../../types/mcp.js';
import { EfiSDK } from '../../index.js';

export const pixDeleteWebhookTool = {
  name: TOOLS_CONFIG.pixDeleteWebhook.name,
  description: TOOLS_CONFIG.pixDeleteWebhook.description,
  parameters: {
    params: z
      .object({
        chave: z
  ...
  handler: async ({
    ...
    params: PixDeleteWebhookParams;
    body: PixDeleteWebhookBody;
  }): Promise<McpResponse> => {
    try {
      const result = await EfiSDK.pixDeleteWebhook(params);
